...
class TestSimpleRead(unittest.TestCase):
    ...
    def test_read_unaligned_tile_boundary_python(self):
        ...
        with bfio.BioWriter(str(TEST_DIR.joinpath("test_output.ome.tiff")), X=img_width, Y=img_height, dtype=np.uint16) as bw:
        ...
        with bfio.BioReader(str(TEST_DIR.joinpath("test_output.ome.tiff"))) as test_br:
            for i in range(100):
                ...
                assert np.sum(source_data[x_start:x_end, y_start:y_end]-test_data) == 0
